...
function applyMerge(existingValue, changes) {
    if (_.isArray(existingValue) || _.isArray(changes)) { return changes }

    if (_.isObject(existingValue) || _.isObject(changes)) {
        // lodash adds a small overhead so we don't use it here
        // eslint-disable-next-line prefer-object-spread, rulesdir/prefer-underscore-method
        const newData = Object.assign({}, fastMerge(existingValue, changes));
...
function merge(key, changes) {
    return get(key)
        .then((existingData) => {
            try {
                const modifiedData = applyMerge(existingData, changes);
